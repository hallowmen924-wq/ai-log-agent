...
import heroOrbit from './assets/hero-orbit.json';
import signalWave from './assets/signal-wave.json';
...
const THEME_OPTIONS = [
  ...
  { value: 'cute', label: '코튼캔디', description: '밝고 귀여운 파스텔 톤' },
];
...
function buildVectorSceneFigure(items, totalCount, palette, animationTick, selectedStoreLabel, selectedVectorTypeLabel, highlightedIds = new Set(), searchLabel = '') {
  ...
  sortedClusters.forEach(([clusterKey, clusterItems], clusterIndex) => {
    ...
    annotations.push({
      ...
      showarrow: false,
      font: { color: palette.text, size: 11, family: 'Space Grotesk' },
  ...
  if (matchedPoints.length) {
    traces.push({
      ...
      textfont: { color: palette.text, size: 10, family: 'Space Grotesk' },
  ...
  return {
    ...
    layout: {
      ...
      title: {
        ...
        font: { color: palette.text, size: 15, family: 'Space Grotesk' },
        x: 0.02,
      },
    },
  };
...
const LOAN_GPT_QUICK_PROMPTS = [
  '오늘 뉴스 기준 카드론 리스크를 요약해줘',
  '승인율을 높일 전환 전략을 정리해줘',
  '지금 운영 현황에서 가장 중요한 포인트가 뭐야?',
];

function createLoanGptWelcomeMessage() {
  return {
    id: 'loan-gpt-welcome',
    role: 'assistant',
    title: '대출GPT',
    text: '안녕하세요. 대출GPT입니다. 운영 현황, 뉴스, 벡터 적재, 카드론 토론실 흐름을 한 번에 묶어서 설명해드릴 수 있습니다. 오른쪽 아래 입력창에 질문을 남기면 현재 화면 기준으로 바로 요약해드리겠습니다.',
    meta: '실시간 컨트롤 타워 연결됨',
  };
}
...
function buildAgentFlowFigure(status) {
  ...
  const traces = [
    ...
    },
    {
      ...
      text: ['MAIN PROCESS'],
      mode: 'text',
      textfont: { color: palette.text, size: 11, family: 'Space Grotesk' },
      hoverinfo: 'skip',
      showlegend: false,
      type: 'scatter',
    },
  ...
  traces.push({
    ...
    textfont: { color: palette.text, size: 12, family: 'IBM Plex Sans KR' },
    marker: {
...
function loadStoredTheme() {
  try {
    ...
    return THEME_OPTIONS.some((item) => item.value === raw) ? raw : 'midnight';
  } catch {
    return 'midnight';
  }
}
...
function App() {
  ...
  const [loanGptMessages, setLoanGptMessages] = useState([createLoanGptWelcomeMessage()]);
  const [loanGptInput, setLoanGptInput] = useState('');
  const [loanGptPending, setLoanGptPending] = useState(false);
  ...
  const loanGptRoomRef = useRef(null);
  const loanGptReplyTimerRef = useRef(null);
  ...
  useEffect(() => () => {
    ...
    if (loanGptReplyTimerRef.current) {
      window.clearTimeout(loanGptReplyTimerRef.current);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      try {
        const [health, chartPayload, summaryPayload, statsPayload] = await Promise.all([fetchHealth(), fetchCharts(), fetchProductSummary(), fetchFaissStats()]);
        if (ignore) {
          return;
        }
        setStatus((previous) => ({
  ...
  useEffect(() => {
    ...
    async function loadVectorSummary() {
      try {
        const activeStore = selectedSection === '대출GPT' ? LOAN_GPT_STORE : selectedStore;
        const payload = await fetchFaissEntriesByStore(120, activeStore, selectedVectorType);
        if (!cancelled) {
  ...
  useEffect(() => {
    const newsCount = (status.news || []).length;
    if (newsCount <= 1) {
      setActiveNewsIndex(0);
      return undefined;
    }
    const intervalId = window.setInterval(() => {
      setActiveNewsIndex((previous) => (previous + 1) % newsCount);
    }, 4500);
    return () => {
      window.clearInterval(intervalId);
    };
  ...
  const isLoanGptSection = selectedSection === '대출GPT';
  const isDebateSection = selectedSection === 'AI 카드론 토론실';
  const isImmersiveSection = isLoanGptSection || isDebateSection;
  const selectedStoreLabel = isLoanGptSection
    ? (STORE_OPTIONS.find((item) => item.value === LOAN_GPT_STORE)?.label || '심사 로그 DB')
    : (STORE_OPTIONS.find((item) => item.value === selectedStore)?.label || '전체 DB');
  ...
  const currentNews = (status.news || []).length ? status.news[activeNewsIndex % status.news.length] : null;
  const visibleNewsItems = (status.news || []).slice(0, 2);
  const visibleRailNewsItems = (status.news || []).slice(0, 1);
  const visibleMainSections = MAIN_SECTIONS.filter((item) => item !== 'Vector DB');
  ...
  function handleSendLoanGptMessage(rawValue) {
    const nextValue = String(rawValue ?? loanGptInput).trim();
    if (!nextValue || loanGptPending) {
      return;
    }

    const messageId = `loan-gpt-user-${Date.now()}`;
    setLoanGptMessages((previous) => ([
      ...previous,
      { id: messageId, role: 'user', title: '나', text: nextValue, meta: '질문 보냄' },
    ]));
    setLoanGptInput('');
    setLoanGptPending(true);

    if (loanGptReplyTimerRef.current) {
      window.clearTimeout(loanGptReplyTimerRef.current);
    }

    loanGptReplyTimerRef.current = window.setTimeout(async () => {
      try {
        const payload = await chatStrategy(nextValue);
        const answer = String(payload?.answer || '').trim() || '응답이 비어 있습니다.';
        const vectorAdded = Number(payload?.vector_update?.added_count || 0);
        setLoanGptMessages((previous) => ([
          ...previous,
          {
            id: `loan-gpt-assistant-${Date.now()}`,
            role: 'assistant',
            title: '대출GPT',
            text: answer,
            meta: `전략 에이전트 응답${vectorAdded ? ` · 벡터 ${vectorAdded}건 적재` : ''}`,
          },
        ]));
        setErrorMessage('');
      } catch (error) {
        ...
        setLoanGptMessages((previous) => ([
          ...previous,
          {
            id: `loan-gpt-assistant-error-${Date.now()}`,
            role: 'assistant',
            title: '대출GPT',
        ...
        setLoanGptPending(false);
      }
    }, prefersReducedMotion ? 0 : 320);
  }

  useEffect(() => {
    const room = loanGptRoomRef.current;
    if (!room) {
      return;
    }
    room.scrollTo({ top: room.scrollHeight, behavior: prefersReducedMotion ? 'auto' : 'smooth' });
  }, [loanGptMessages, loanGptPending, prefersReducedMotion]);
  ...
  const renderContent = () => {
    if (selectedSection === '대출GPT') {
      return (
        <motion.section key="loan-gpt" className="loan-gpt-shell" {...sectionTransition}>
          <motion.div className="loan-gpt-stage observatory-stage" initial={{ opacity: 0, scale: 0.985 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.99 }} transition={{ duration: 0.28, ease: 'easeOut' }}>
            <div className="loan-gpt-header observatory-header">
              <div className="observatory-header-copy">
                <div className="panel-kicker">FAISS Observatory</div>
                <h2>로그 군집 놀이터</h2>
                <p>이제 이 영역은 그래프 대신 검색 결과만 보여줍니다. 심사 로그 DB에서 상품명, type, reject code, 로그 문구로 바로 찾고, 결과 카드에서 FULLTEXT를 열람할 수 있게 단순화했습니다.</p>
              </div>
              <div className="loan-gpt-status-row observatory-status-row">
                <span className="sample-pill">logs only</span>
                <span className="sample-pill">전체 {vectorSceneInsights.totalCount.toLocaleString()} vectors</span>
                <span className="sample-pill">로드 {vectorSceneInsights.loadedCount.toLocaleString()} samples</span>
                <span className="sample-pill">주력 type {vectorSceneInsights.dominantType}</span>
              </div>
            </div>
            <div className="loan-gpt-quick-row observatory-quick-row">
              <span className="sample-pill">store {selectedStoreLabel}</span>
              <span className="sample-pill">type {selectedVectorTypeLabel}</span>
              <span className="sample-pill">richest {vectorSceneInsights.richestVector}</span>
              <span className="sample-pill">검색 결과 카드만 표시</span>
              {vectorSearchMatches.length ? <span className="sample-pill">검색 매치 {vectorSearchMatches.length}개</span> : null}
              {vectorSearchBusy ? <span className="sample-pill">FAISS 검색 중</span> : null}
            </div>
            <MotionCard
              className="panel observatory-results-panel"
              style={{
                minHeight: 700,
                padding: 16,
                background: theme === 'cute'
                  ? 'linear-gradient(180deg, rgba(255,255,255,0.96), rgba(250,244,250,0.94))'
                  : 'linear-gradient(180deg, rgba(9,23,35,0.96), rgba(6,17,27,0.94))',
              }}
            >
              <div className="panel-head panel-head-spread observatory-search-head">
                <div>
                  <div className="panel-kicker">Similarity Finder</div>
                  <h2>유사 로그 검색</h2>
                </div>
                <form className="observatory-search-form" onSubmit={handleVectorSearchSubmit}>
                  <input
                    className="vector-search-input"
                    type="text"
                    value={vectorSearchQuery}
                    onChange={(event) => setVectorSearchQuery(event.target.value)}
                    placeholder="상품명, type, reject code, 로그 문구로 검색"
                  />
                  <button className="primary-button" type="submit" disabled={vectorSearchBusy}>{vectorSearchBusy ? '검색 중' : '검색'}</button>
                  <button className="secondary-button" type="button" onClick={handleClearVectorSearch} disabled={vectorSearchBusy}>초기화</button>
                </form>
              </div>
              <div className="observatory-result-summary-row">
                <span className="sample-pill">검색어 {normalizedVectorSearchQuery || '미입력'}</span>
                <span className="sample-pill">결과는 로그 DB 유사도 순</span>
                <span className="sample-pill">카드 클릭 시 FULLTEXT</span>
              </div>
              {vectorSearchMatches.length ? (
                <div className="observatory-result-list">
                  {vectorSearchMatches.map((item, index) => (
                    <button
                      key={item.id}
                      type="button"
                      className="observatory-result-card"
                      onClick={() => handleOpenVectorDetail(item)}
                    >
                      <div className="observatory-result-card-head">
                        <div>
                          <div className="panel-kicker">Result {String(index + 1).padStart(2, '0')}</div>
                          <h3>{truncate(item.name || item.id, 48)}</h3>
                        </div>
                        <span className="observatory-result-score">faiss {Number(item.score || item.similarityScore || 0).toFixed(3)}</span>
                      </div>
                      <div className="observatory-result-meta">
                        <span className="sample-pill">store {item.store || '-'}</span>
                        <span className="sample-pill">type {item.type || '-'}</span>
                        <span className="sample-pill">product {item.product || '-'}</span>
                        <span className="sample-pill">reject {(item.reject_reason_codes || []).length}개</span>
                        <span className="sample-pill">features {Object.keys(item.features || {}).length}개</span>
                      </div>
                      <p>{truncate(item.snippet || '본문 요약이 없습니다.', 240)}</p>
                    </button>
                  ))}
                </div>
              ) : (
                <div className="empty-box observatory-results-empty" style={{ minHeight: 420 }}>
                  {normalizedVectorSearchQuery
                    ? '검색 결과가 없습니다. 다른 상품명, type, reject code, 로그 문구로 다시 시도해보세요.'
                    : '검색어를 입력하면 심사 로그 DB에서 유사한 결과 카드만 표시합니다.'}
                </div>
              )}
            </MotionCard>
          </motion.div>
      ...
      return (
        <motion.section key="debate" className="debate-fullscreen-shell" {...sectionTransition}>
          ...
          <MotionCard className="debate-hero">
            ...
            <Lottie animationData={signalWave} loop autoplay className="debate-wave" style={{ width: '100%', height: '100%', display: 'block' }} {...LOTTIE_SVG_PROPS} />
    ...
    if (selectedSection === '대출상품 Dashboard') {
      return (
        <motion.section key="dashboard" className="content-stack" {...sectionTransition}>
          <MotionCard className="panel">
            <div className="panel-head"><div><div className="panel-kicker">FAISS Summary</div><h2>상품별 승인/거절 패턴이 이렇게 요약돼 들어갑니다</h2></div><p>data/product_pattern_summary.json을 백엔드 API로 읽어와 현재 로그 요약과 같은 화면 구조로 보여줍니다.</p></div>
            <motion.div className="product-grid" variants={containerVariants} initial="hidden" animate="show">
              {summaryProducts.map(([productCode, payload]) => {
                ...
                return (
                  <MotionCard className="product-card" as="article" key={productCode} whileHover={liftCard({ y: -8, rotateX: -1, rotateY: 1 })}>
                    <div className="product-card-head"><div><div className="product-code">{productCode}</div><h3>{payload?.product_name || productCode}</h3></div><span className="sample-pill">의사결정 표본 {totals.decision_known_cases || 0}건</span></div>
                    <div className="signal-grid">
                      <div className="signal-box approve"><span>승인 시그널</span><strong>{totals.approval_rate_percent || 0}%</strong><small>{totals.approval_cases || 0}건 승인</small></div>
                      <div className="signal-box reject"><span>거절 시그널</span><strong>{totals.rejection_rate_percent || 0}%</strong><small>{totals.rejection_cases || 0}건 거절</small></div>
                    </div>
                    <div className="product-lists">
                      <div><h4>승인으로 기우는 패턴</h4><ul>{approvalPatterns.slice(0, 3).map((item, index) => <li key={`approve-${index}`}>{truncate(item.rule || '-', 74)}</li>)}{!approvalPatterns.length ? <li>뚜렷한 승인 패턴 없음</li> : null}</ul></div>
                      <div><h4>거절로 기우는 패턴</h4><ul>{rejectionPatterns.slice(0, 3).map((item, index) => <li key={`reject-${index}`}>{truncate(item.rule || '-', 74)}</li>)}{!rejectionPatterns.length ? <li>뚜렷한 거절 패턴 없음</li> : null}</ul></div>
                    </div>
                    <div className="reason-chip-row">
                      {rejectReasons.slice(0, 3).map((item, index) => <span className="reason-chip" key={`reason-${index}`}>{truncate(`${item.code || ''} ${item.description || ''}`.trim(), 42)}</span>)}
  ...
  return (
    <div className={`app-shell ${isImmersiveSection ? 'loan-gpt-mode' : ''} theme-${theme}`}>
      <div className="theme-toolbar" ref={themeMenuRef}>
        ...
        >
          ...
          <span className="theme-toggle-copy">
            <strong>옵션</strong>
            <small>{theme === 'cute' ? '코튼캔디 테마' : '미드나잇 테마'}</small>
          </span>
        </button>
        <AnimatePresence>
          {themeMenuOpen ? (
            ...
            <div className="hero-visual hero-visual-layered">
              <motion.div className="hero-visual-shell hero-visual-shell-title" animate={prefersReducedMotion ? undefined : { y: [0, -6, 0] }} transition={{ duration: 4.6, repeat: Infinity, ease: 'easeInOut' }}>
                ...
                <div className="hero-wave-layer">
                  <Lottie animationData={signalWave} loop autoplay className="rail-lottie" style={{ width: '100%', height: '100%', display: 'block' }} {...LOTTIE_SVG_PROPS} />
                </div>
                <div className="hero-orbit-layer">
                  <Lottie animationData={heroOrbit} loop autoplay className="hero-lottie" style={{ width: '100%', height: '100%', display: 'block' }} {...LOTTIE_SVG_PROPS} />
                </div>
      ...
      <div className={`page-grid ${isImmersiveSection ? 'loan-gpt-layout' : ''}`}>
        <AnimatePresence>
          {!isImmersiveSection ? (
            <motion.aside className="left-rail" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -24 }} transition={{ duration: 0.35, ease: 'easeOut' }}>
          <MotionCard className="rail-panel compact-rail-panel compact-news-panel">
            <div className="rail-panel-glow rail-panel-glow-news" />
            ...
            <div className="news-live-indicator">
              ...
              <span>{currentNews ? `최근 포커스 ${Math.min(activeNewsIndex + 1, (status.news || []).length)} / ${(status.news || []).length}` : '수집 대기 중'}</span>
            </div>
            <div className="news-queue-shell">
              <AnimatePresence initial={false}>
                {visibleRailNewsItems.length ? visibleRailNewsItems.map((item, index) => {
                  const absoluteIndex = index;
                  const isActive = visibleRailNewsItems.length ? absoluteIndex === activeNewsIndex % visibleRailNewsItems.length : false;
                  return (
                    <motion.article
                      key={`${item.link || item.title || 'news'}-${absoluteIndex}`}
                      className={`news-card news-card-queue ${isActive ? 'active' : ''}`}
                      initial={prefersReducedMotion ? { opacity: 0 } : { opacity: 0, y: 16, scale: 0.98 }}
                      animate={prefersReducedMotion ? { opacity: 1 } : { opacity: 1, y: 0, scale: isActive ? 1.01 : 1 }}
                      exit={prefersReducedMotion ? { opacity: 0 } : { opacity: 0, y: -12, scale: 0.98 }}
                      transition={{ duration: 0.38, ease: 'easeOut', delay: index * 0.05 }}
                    >
                      <div className="news-card-flow" />
                      <div className="news-card-headline-row">
                        <span className={`news-rank-badge ${isActive ? 'active' : ''}`}>#{absoluteIndex + 1}</span>
                        <span className={`news-status-chip ${isActive ? 'active' : ''}`}>{isActive ? '새 소식 감지' : '모니터링 중'}</span>
                      </div>
                      <div className="news-card-meta"><span>{truncate(item.source || 'news', 28)}</span><span>{formatTime(item.published_at || item.collected_at || status.last_news_time)}</span></div>
                      <h3>{truncate(item.title || '제목 없음', 74)}</h3>
                      <p className="news-card-copy">{truncate(item.content || item.summary || '본문 미리보기가 없습니다.', 108)}</p>
                      <div className="news-rotator-footer">
                        <span className="news-rotator-count">{isActive ? '업데이트 강조 중' : '대기열 유지'}</span>
                        {item.link ? <a href={item.link} target="_blank" rel="noreferrer">원문 열기</a> : null}
                      </div>
                    </motion.article>
                  );
                }) : (
                  <motion.div key="empty-news" className="empty-box" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                    표시할 뉴스가 아직 없습니다.
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
            {visibleRailNewsItems.length > 1 ? (
              <div className="news-rotator-dots" aria-hidden="true">
                {visibleRailNewsItems.map((item, index) => (
                  <span key={`${item.link || item.title || 'dot'}-${index}`} className={`news-dot ${index === activeNewsIndex % visibleRailNewsItems.length ? 'active' : ''}`} />
                ))}
              </div>
            ) : null}
          </MotionCard>
          <MotionCard className="rail-panel upload-panel compact-rail-panel compact-upload-panel">
            <div className="rail-panel-glow rail-panel-glow-regulation" />
            <div className="panel-kicker">Regulation Intake</div>
            <h2>규제 문서 업로드</h2>
            <p className="panel-copy">규제 문서를 바로 올리고 즉시 학습 상태만 빠르게 확인할 수 있게 압축했습니다.</p>
            ...
            <div className="regulation-stage-shell">
              <motion.div
                className={`upload-dropzone-visual ${regulationBusy ? 'busy' : ''}`}
                whileHover={liftCard({ scale: 1.01 })}
                onClick={openRegulationFilePicker}
                onKeyDown={(event) => {
                  if (event.key === 'Enter' || event.key === ' ') {
                    event.preventDefault();
                ...
                role="button"
                tabIndex={0}
                aria-label="규제 문서 파일 첨부"
              >
                <div className="upload-dropzone-grid" />
                <motion.div className="upload-dropzone-beam" animate={prefersReducedMotion ? undefined : { y: ['-78%', '78%'] }} transition={{ duration: 2.7, repeat: Infinity, ease: 'linear' }} />
                <motion.div className="upload-dropzone-core" animate={prefersReducedMotion ? undefined : { scale: [1, 1.05, 1], opacity: [0.82, 1, 0.82] }} transition={{ duration: 2.2, repeat: Infinity, ease: 'easeInOut' }}>
                  <span className="upload-dropzone-arrow">↑</span>
                  <strong>{regulationBusy ? 'UPLOADING...' : 'DROP FILES'}</strong>
                  <span>{regulationBusy ? '선택 즉시 AI 규제 문서 학습을 실행하고 있습니다.' : '클릭하면 바로 파일을 첨부하고 즉시 학습을 시작합니다.'}</span>
                </motion.div>
              </motion.div>
              <div className="upload-doc-cluster" aria-hidden="true">
                <div className="upload-doc-card pdf">
                  <div className="upload-doc-orbit" />
                  <div className="upload-doc-label">PDF</div>
                  <div className="upload-doc-lines"><span /><span /><span /></div>
                  <div className="upload-doc-progress">SCAN</div>
                  <div className="upload-doc-check">✓</div>
                </div>
                <div className="upload-doc-card word">
                  <div className="upload-doc-orbit" />
                  <div className="upload-doc-label">DOC</div>
                  <div className="upload-doc-lines"><span /><span /><span /></div>
                  <div className="upload-doc-progress">QUEUE</div>
                  <div className="upload-doc-check">✓</div>
                </div>
              </div>
            </div>
            <div className="status-pill-row"><span className={`status-pill ${getStatusPalette(status.agent_statuses?.regulation_agent?.status).tone}`}>{getStatusPalette(status.agent_statuses?.regulation_agent?.status).label}</span><span className="status-pill neutral">최근 {formatTime(status.agent_statuses?.regulation_agent?.updated_at)}</span></div>
            <div className="summary-box"><div className="summary-box-title">최신 규제 요약</div><p>{truncate(status.latest_regulation_analysis || status.agent_statuses?.regulation_agent?.detail || '업로드된 규제 요약이 없습니다.', 260)}</p></div>
          </MotionCard>
            </motion.aside>
          ) : null}
        </AnimatePresence>
        <motion.main className={`main-rail ${isImmersiveSection ? 'expanded' : ''}`} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, ease: 'easeOut', delay: 0.08 }}>
          <motion.div className="section-tabs" variants={containerVariants} initial="hidden" animate="show">
            {visibleMainSections.map((item) => <motion.button key={item} type="button" className={`section-tab ${selectedSection === item ? 'active' : ''}`} onClick={() => setSelectedSection(item)} variants={cardVariants} whileHover={liftCard({ y: -3, scale: 1.01 })} whileTap={prefersReducedMotion ? undefined : { scale: 0.985 }}><span className="section-tab-pulse" /><span>{item}</span></motion.button>)}
          </motion.div>
