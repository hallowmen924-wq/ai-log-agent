...
FAISS_SEARCH_CACHE_TTL_SECONDS = 20
FAISS_SEARCH_CACHE_LIMIT = 64

_faiss_search_cache: collections.OrderedDict[tuple[str | None, str, int, tuple[tuple[str, int], ...]], dict[str, object]] = collections.OrderedDict()
_faiss_search_cache_lock = threading.Lock()
...
class _LazyLockProxy:
    def __init__(self, lock_getter):
        self._lock_getter = lock_getter

    def __enter__(self):
        return self._lock_getter().__enter__()

    def __exit__(self, exc_type, exc_value, traceback):
        return self._lock_getter().__exit__(exc_type, exc_value, traceback)

    def acquire(self, *args, **kwargs):
        return self._lock_getter().acquire(*args, **kwargs)

    def release(self):
        return self._lock_getter().release()


OLLAMA_EXECUTION_LOCK = _LazyLockProxy(
    lambda: _strategy_chat_module().OLLAMA_EXECUTION_LOCK
)
...
def run_periodic_log_agent(*args, **kwargs):
    return _strategy_chat_module().run_periodic_log_agent(*args, **kwargs)


def run_periodic_news_agent(*args, **kwargs):
    return _strategy_chat_module().run_periodic_news_agent(*args, **kwargs)


def strategy_chat(*args, **kwargs):
    return _strategy_chat_module().strategy_chat(*args, **kwargs)


def append_news_documents(*args, **kwargs):
    return _vector_db_module().append_news_documents(*args, **kwargs)


def append_structured_log_documents(*args, **kwargs):
    return _vector_db_module().append_structured_log_documents(*args, **kwargs)


def build_vector_db(*args, **kwargs):
    return _vector_db_module().build_vector_db(*args, **kwargs)


def search_logs_context(*args, **kwargs):
    return _vector_db_module().search_logs_context(*args, **kwargs)


def get_store_path(*args, **kwargs):
    store_name = str((args[0] if args else kwargs.get("store_name") or "")).strip().lower()
    if store_name == FAISS_STORE_LOGS:
        return str(_SERVICE_ROOT / "faiss_logs")
    if store_name == FAISS_STORE_NEWS:
        return str(_SERVICE_ROOT / "faiss_news")
    if store_name == FAISS_STORE_CUSTOMER:
        return str(_SERVICE_ROOT / "faiss_customer")
    if store_name == FAISS_STORE_DOCUMENT:
        return str(_SERVICE_ROOT / "faiss_document")
    return _vector_db_module().get_store_path(*args, **kwargs)


def get_store_news_keys(*args, **kwargs):
    return _vector_db_module().get_store_news_keys(*args, **kwargs)


def get_store_document_count(*args, **kwargs):
    return _vector_db_module().get_store_document_count(*args, **kwargs)


def _get_faiss_store_signature(store_name: str) -> tuple[str, int]:
    ...
    for candidate in (index_path, metadata_path):
        try:
            ...
            continue
    return store_name, latest_mtime_ns


def _build_faiss_cache_signature(store_name: str | None) -> tuple[tuple[str, int], ...]:
    if store_name == FAISS_STORE_LOGS:
        stores = (FAISS_STORE_LOGS,)
    elif store_name == FAISS_STORE_NEWS:
        stores = (FAISS_STORE_NEWS, FAISS_STORE_DOCUMENT)
    elif store_name == FAISS_STORE_DOCUMENT:
        stores = (FAISS_STORE_DOCUMENT,)
    elif store_name == FAISS_STORE_CUSTOMER:
        stores = (FAISS_STORE_CUSTOMER,)
    else:
        stores = (FAISS_STORE_LOGS, FAISS_STORE_NEWS, FAISS_STORE_DOCUMENT)
    return tuple(_get_faiss_store_signature(current_store) for current_store in stores)


def _get_cached_faiss_response(cache_key: tuple[str | None, str, int, tuple[tuple[str, int], ...]]) -> dict[str, list[str]] | None:
    now = time.time()
    with _faiss_search_cache_lock:
        expired_keys = [
            key
            for key, value in _faiss_search_cache.items()
            if now - float(value.get("cached_at") or 0.0) > FAISS_SEARCH_CACHE_TTL_SECONDS
        ]
        for key in expired_keys:
            _faiss_search_cache.pop(key, None)
        cached = _faiss_search_cache.get(cache_key)
        if cached is None:
            return None
        _faiss_search_cache.move_to_end(cache_key)
        return dict(cached.get("payload") or {})


def _store_cached_faiss_response(cache_key: tuple[str | None, str, int, tuple[tuple[str, int], ...]], payload: dict[str, list[str]]) -> None:
    with _faiss_search_cache_lock:
        _faiss_search_cache[cache_key] = {
            "cached_at": time.time(),
            "payload": dict(payload),
        }
        _faiss_search_cache.move_to_end(cache_key)
        while len(_faiss_search_cache) > FAISS_SEARCH_CACHE_LIMIT:
            _faiss_search_cache.popitem(last=False)
...
def list_vectors(*args, **kwargs):
    return _vector_db_module().list_vectors(*args, **kwargs)


def prepare_log_ingest_preview(*args, **kwargs):
    return _vector_db_module().prepare_log_ingest_preview(*args, **kwargs)
...
def search_customer_context(*args, **kwargs):
    return _vector_db_module().search_customer_context(*args, **kwargs)


def search_news_context(*args, **kwargs):
...
class BackendState:
    ...
    def snapshot(
        self,
        include_faiss_items: bool = True,
        include_vector_count: bool = True,
    ) -> dict[str, Any]:
        if include_vector_count:
            try:
...
TEST_LOG_BURST_COUNT = max(1, int(os.environ.get("WORKER_LOG_BURST_COUNT", "1") or 1))
...
def merge_news_items(
    ...
    # 제목/링크를 정규화해서 사실상 같은 기사면 하나만 유지합니다.
    from agent.news_agent import _build_news_identity_keys

    merged: list[dict[str, Any]] = []
    seen: set[tuple[str, str]] = set()
    existing_keys: set[tuple[str, str]] = set()
    for item in existing_news:
        existing_keys.update(_build_news_identity_keys(item))
    new_unique_count = 0
    ...
    for item in list(new_news) + list(existing_news):
        identity_keys = _build_news_identity_keys(item)
        if not identity_keys:
            continue
        if identity_keys & seen:
            continue
        if not (identity_keys & existing_keys):
...
def run_background_news_agent_cycle(should_persist: bool = True) -> dict[str, Any]:
    ...
    result = run_periodic_news_agent(
        ...
        ollama_enabled=bool(state.news_agent_ollama_enabled),
    )
    with state.lock:
...
def generate_test_log_cycle(record_event: bool = True) -> dict[str, Any]:
    ...
    generated = append_synthetic_log(product)
    generated_at = datetime.datetime.now()
    with state.lock:
...
def run_background_log_agent_cycle(should_persist: bool = True) -> dict[str, Any]:
    ...
    result = run_periodic_log_agent(
        ...
        ollama_enabled=bool(state.log_agent_ollama_enabled),
    )
    with state.lock:
...
def search_faiss(query: str, k: int = 5, store_name: str | None = None) -> dict[str, list[str]]:
    ...
    cache_key = (
        ...
        normalized_query,
        int(k or 5),
        _build_faiss_cache_signature(normalized_store),
    )
    cached = _get_cached_faiss_response(cache_key)
    if cached is not None:
        return cached

    if normalized_store == FAISS_STORE_LOGS:
        payload = {"logs": search_logs_context(normalized_query, k=k), "news": [], "rules": [], "customer": []}
    elif normalized_store == FAISS_STORE_NEWS:
        news, rules = search_news_context(normalized_query, k=k)
        payload = {"logs": [], "news": news, "rules": rules, "customer": []}
    elif normalized_store == FAISS_STORE_DOCUMENT:
        _, rules = search_news_context(normalized_query, k=k)
        payload = {"logs": [], "news": [], "rules": rules, "customer": []}
    elif normalized_store == FAISS_STORE_CUSTOMER:
        customer = search_customer_context(normalized_query, k=k)
        payload = {"logs": [], "news": [], "rules": [], "customer": customer}
    else:
        logs, news, rules = search_context(normalized_query, k=k)
        payload = {"logs": logs, "news": news, "rules": rules, "customer": []}

    _store_cached_faiss_response(cache_key, payload)
    return payload
