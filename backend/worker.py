...
class NewsVectorWorker:
    ...
    def _recalculate_task_intervals(self) -> None:
        ...
        self.log_cycle_seconds = _env_int("WORKER_LOG_CYCLE_SECONDS", 60, minimum=10)
        self.log_burst_count = _env_int("WORKER_LOG_BURST_COUNT", 1, minimum=1)
        self.log_analysis_seconds = _env_int(
            "WORKER_LOG_ANALYSIS_SECONDS",
            max(60, self.log_cycle_seconds),
            minimum=10,
        )
        self.log_agent_seconds = _env_int(
            "WORKER_LOG_AGENT_SECONDS",
            max(180, base * 30),
            minimum=30,
        )
        self.news_cycle_seconds = _env_int("WORKER_NEWS_CYCLE_SECONDS", 60, minimum=30)
        self.faiss_cycle_seconds = _env_int(
            "WORKER_FAISS_CYCLE_SECONDS",
            max(600, base * 120),
            minimum=180,
        )
